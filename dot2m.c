...
/*
    ...
    variable mutexsum, and unlock it upon updating.
    When you call, pass its address of mutexsum.*/
    
 //2.2A - Place here your code to lock the mutex (myMutex)
 // 2.2A - Lock the named mutex (myMutex)
    ...
    // 2.2B - Place here your code to unlock the mutex (myMutex)
    // 2.2B - Unlock the named mutex (myMutex)
...
// Initialize the named mutex (myMutex)
    int protection = PROT_READ | PROT_WRITE;
    int visibility = MAP_ANONYMOUS | MAP_SHARED;
    myMutex = (pthread_mutex_t *)mmap(0, sizeof(pthread_mutex_t), protection, visibility, -1, 0);
